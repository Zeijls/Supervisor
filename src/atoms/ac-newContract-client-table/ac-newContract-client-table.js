...
// Imports => Constants
import { ROUTES, COLUMNS } from '@constants';
...
const _CLASSES = {
  ...
  SEARCHBOX: {
    MAIN: 'ac-sorting__searchbox',
    TITLE: 'ac-sorting__searchbox__title',
  },
};

const AcNewContractClientTable = ({
  onClick,
  is_loading,
  handlePagination,
  handleSearch,
  ...
  const getSearchBoxClassNames = useMemo(() => {
    return clsx(_CLASSES.SEARCHBOX.MAIN);
  }, []);

  const getSearchBoxTitleClassNames = useMemo(() => {
    return clsx(_CLASSES.SEARCHBOX.TITLE);
  }, []);

  const renderSearchBar = useMemo(() => {
    return (
      <div className={getSearchBoxClassNames}>
        <h5 className={getSearchBoxTitleClassNames}>
  ...
  const renderTable = useMemo(() => {
    ...
    // if (store.clients.is_loading) return <AcLoader loading={true} />;
    if (!client_data) return null;
    const { data, meta } = client_data;
    if (!data || data.length === 0) return null;
    const options = {
      columns: COLUMNS.CLIENTS,
      rows: data,
      pagination: meta,
      // sortby: store.clients.current_order_by,
      onSort: handleSort,
      onPaginate: null,
      onRowClick: (object) => {
        if (!object) return;
...
export default AcNewContractClientTable;
