...
const _CLASSES = {
  ...
  NOSEARCHRESULTS: {
    MAIN: 'ac-overview__no-results',
    TITLE: 'ac-overview__no-results__title',
    SVG: 'ac-overview__no-results__svg',
  },
};

const AcContractOverview = ({ store }) => {
  ...
  const { contract_data, table_data } = store.contracts;
  const { search_image } = store.contracts;
  ...
  const getNoSearchResultClassNames = useMemo(() => {
    return clsx(_CLASSES.NOSEARCHRESULTS.MAIN);
  }, []);

  const getTitleClassNames = useMemo(() => {
    return clsx(_CLASSES.NOSEARCHRESULTS.TITLE);
  }, []);

  const getSvgClassNames = useMemo(() => {
    return clsx(_CLASSES.NOSEARCHRESULTS.SVG);
  }, []);
  ...
  const renderPageTitle = useMemo(() => {
    const collection = [
      ...
      {
        label: 'Contractenoverzicht',
      },
    ];

    return (
      <AcPageTitle title={'Contracten overzicht'} collection={collection} />
    );
  }, []);
  ...
  const renderHeader = useMemo(() => {
    return (
      <div className={getHeaderClassNames}>
        <AcSearchBar onChange={handleSearch} />
  ...
  const renderTable = useMemo(() => {
    if (store.contracts.is_loading) return <AcLoader loading={true} />;
    if (!contract_data) return null;
    const { meta } = contract_data;
    ...
    if (table_data) {
      ...
      const options = {
        ...
        rows: table_data,
        pagination: meta,
        sortby: store.contracts.current_sort,
        onSort: handleSort,
        ...
        onRowClick: (object) => {
          ...
          const { push } = history;
          if (push && id) {
            const route = ROUTES.CONTRACTDETAIL.path.replace(':id', id);
            push(route);
          }
        },
      };
    ...
    contract_data,
    table_data,
    store.contracts.is_loading,
    store.contracts.current_sort,
  ]);

  const checkSearchResults = useMemo(() => {
    if (search_image) {
      return (
        <div className={getNoSearchResultClassNames}>
          <svg
            className={getSvgClassNames}
  ...
  const renderContractOverviewPage = useMemo(() => {
    return (
  ...
  return <div className={getMainClassNames}>{renderContractOverviewPage}</div>;
};

export default withStore(observer(AcContractOverview));
