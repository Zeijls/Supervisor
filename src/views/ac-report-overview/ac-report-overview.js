...
const AcReportOverview = ({ store }) => {
  const history = useHistory();
  const { handlePagination, handleSort, handleSearch } = useOverviewActions(
    store.report
  );

  const { report_data, table_data } = store.report;
  const { search_image } = store.report;
  ...
  const renderPageTitle = useMemo(() => {
    const collection = [
      ...
      {
        label: 'Rapportageoverzicht',
      },
    ];

    return (
      <AcPageTitle title={'Rapportage overzicht'} collection={collection} />
    );
  }, []);
  ...
  const renderTable = useMemo(() => {
    ...
    if (table_data) {
      console.log(table_data, 'table data');
...
export default withStore(observer(AcReportOverview));
